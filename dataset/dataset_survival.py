...
class Generic_MIL_Survival_Dataset(Generic_WSI_Survival_Dataset):
    def __init__(self, data_dir=None, mode='omic', feature_type='titan', use_h5=False, **kwargs):
        self.use_h5 = use_h5  # 在super()之前设置
        super(Generic_MIL_Survival_Dataset, self).__init__(mode=mode, **kwargs)
        # 支持多个特征目录
        if isinstance(data_dir, str):
            self.data_dir = {'titan': data_dir}
        else:
    ...
    def __getitem__(self, idx):
        ...
        #print(self.mode)
        #print(self.data_dir)
        #raise NotImplementedError('hhh' % self.mode)
            ...
            if self.data_dir:
                if self.mode == 'multimodal':
                    ...
                    for slide_id in slide_ids:
                        titan_path = os.path.join(self.data_dir['titan'], '{}.h5'.format(slide_id.rstrip('.svs')))
                        with h5py.File(titan_path, 'r') as hdf5_file:
                            feat = torch.from_numpy(hdf5_file['features'][:])
                            # Titan特征是768维单向量
                            titan_features.append(feat)
                    titan_features = torch.stack(titan_features, dim=0)

                    # 读取Uni特征
                    uni_features = []
                    for slide_id in slide_ids:
                        uni_path = os.path.join(self.data_dir['uni'], '{}.h5'.format(slide_id.rstrip('.svs')))
                        with h5py.File(uni_path, 'r') as hdf5_file:
                            feat = torch.from_numpy(hdf5_file['features'][:])
                            # Uni特征是(N, 1536)矩阵
                            uni_features.append(feat)
                    uni_features = torch.cat(uni_features, dim=0)

                    return (titan_features, uni_features, label, event_time, c)
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, mode, signatures=None, data_dir=None, label_col=None, patient_dict=None, num_classes=2, use_h5=True):
        self.use_h5 = use_h5  # 直接设置use_h5
        self.slide_data = slide_data
