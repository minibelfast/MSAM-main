...
def main(args):
    ...
    for i in folds:
        ...
        if args.pretrained_model:
            ...
            args.lr = args.finetune_lr  # 使用更小的学习率进行微调
...
parser.add_argument('--data_root_dir_titan', type=str, default="./data/features_titan_h5",
                    help='Data directory to Titan features')
parser.add_argument('--data_root_dir_uni', type=str, default="./data/features_uni_h5",
                    help='Data directory to Uni features')
parser.add_argument('--use_h5', action='store_true', default=True,
                    help='Use h5 files instead of pt files')
parser.add_argument('--data_root_dir', type=str, default="./data/clam_features",
...
parser.add_argument('--split_dir', type=str, default='./splits/TCGA_BLCA_survival_kfold',
...
parser.add_argument('--model_type', type=str, choices=['mean_mil', 'max_mil', 'att_mil','trans_mil', 's4model','mamba_mil','mamba_attn',"msa_mil","blca_mil"], default='mamba_attn', 
                    help='type of model')
parser.add_argument('--mode', type = str, choices=['multimodal','path', 'omic', 'pathomic', 'cluster'], default='path', help='which modalities to use')
...
parser.add_argument('--exp_code', type=str, default='transmil_uni', help='experiment code for saving results')
parser.add_argument('--weighted_sample', action='store_true', default=True, help='enable weighted sampling')
parser.add_argument('--task', default='TCGA_BLCA_survival', type=str)
...
parser.add_argument('--freeze_except_adapter', action='store_true', default=False, 
                    help='Whether to freeze all parameters except mag_adapter')

# 修改知识蒸馏相关参数
parser.add_argument('--distillation', action='store_true', default=False, help='是否使用知识蒸馏')
parser.add_argument('--teacher_model_paths', nargs='+', type=str, default=[], help='教师模型的权重路径列表')
parser.add_argument('--teacher_weights', nargs='+', type=float, default=[0.5, 0.5], help='每个教师模型的权重')
parser.add_argument('--temperature', type=float, default=4.0, help='知识蒸馏的温度参数')
parser.add_argument('--alpha_kd', type=float, default=0.5, help='知识蒸馏损失的权重')

# 添加 EWC 相关参数
parser.add_argument('--use_ewc', action='store_true', default=False, help='是否使用EWC')
parser.add_argument('--ewc_lambda', type=float, default=1.0, help='EWC损失的权重')
...
if 'survival' in args.task:
    ...
    data_dir = {
        'titan': os.path.join(args.data_root_dir_titan),
        'uni': os.path.join(args.data_root_dir_uni)
    }
    dataset = Generic_MIL_Survival_Dataset(
        csv_path = 'dataset_csv/%s_processed.csv' % combined_study,
        mode = 'multimodal',  # 改为multimodal模式
        apply_sig = args.apply_sig,
        data_dir = data_dir,  # 使用新的数据目录格式
        shuffle = True, 
        seed = args.seed, 
        print_info = True,
        patient_strat = False,
