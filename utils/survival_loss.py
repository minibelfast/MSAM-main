...
def nll_loss(hazards, survival, Y, c, alpha=0.4, eps=1e-7):
    ...
    uncensored_loss = -(1 - c) * torch.log(1 - torch.gather(survival_padded, 1, Y).clamp(min=eps))  
    censored_loss = -c * torch.log(1 - torch.gather(survival_padded, 1, Y + 1).clamp(min=eps)) 

    death_prob_loss = (1 - alpha) * (uncensored_loss + censored_loss)  
    loss = death_prob_loss.mean()
    loss = loss.mean()
    return loss
