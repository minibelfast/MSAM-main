...
parser.add_argument('--model_path', type=str, default='/mnt/data3/heatmap/slide/s_3_checkpoint_tm.pth') 
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        with torch.no_grad():
            ...
            for layer in model.layers:
                ...
                h = h + h_ 
            h = model.norm(h) 
            A = model.classifier(h)  # [B, n_classes]
            #A = torch.topk(A, 1, dim=2)[0]
            #print(A.shape)
            #raise ValueError("已通过")
            A = A[:, :, 3].unsqueeze(2)
            _, _, Y_hat, A2, Y_prob = model(features)
            #print(A.shape)
            #print(A2.shape)
            #raise ValueError("已通过")
            Y_hat = Y_hat.item()
