...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        with torch.no_grad():
            ...
            for layer in model.layers2:
                ...
                B = B + B_
            B = model.normA(B)
            outputs = model.DFF(A.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1),B.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1).permute(0, 2, 1)#给特征加权重.permute(0, 2, 1, 3)
            #print(outputs.shape)
            #raise ValueError("已通过")
    ...
    def plot_clusters(data, cluster_labels, palette, output_dir, output_filename):
        ...
        for i in range(len(palette)):
            plt.scatter(data[cluster_labels == i, 0], data[cluster_labels == i, 1], 
                        label=f'Cluster {i}', marker='o', s=2, color=palette[i])
    ...
    palette = ['#6252B4', '#4D53B3', '#4F8EBB', '#64A49F', '#ABBC86', '#DAAE6E', '#B46257', '#B75369']
