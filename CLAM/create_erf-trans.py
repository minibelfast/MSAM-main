...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        # ---->cls_token
        cls_tokens = model.cls_token.expand(1, -1, -1).cuda()
        h = torch.cat((cls_tokens, h), dim=1)
        h = model.layer1(h)  # [B, N, 256]

        # ---->PPEG
        h = model.pos_layer(h, _H, _W)  # [B, N, 256]

        # ---->Translayer x2
        h = model.layer2(h)  # [B, N, 256]
        ...
        outputs = h.unsqueeze(-1).unsqueeze(0)
        #print(outputs.shape)
