...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(process_stack))):
        ...
        #optimizer.zero_grad()
        features = features.expand(1, -1, -1)
        features = features.float()
        features = model._fc1(features)  # [B, n, 512]
        ...
        with torch.no_grad():
            ...
            for layer in model.layers2:
                ...
                B = B + B_
            B = model.normA(B)
            #feature = model.DFF(A.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1),B.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1).permute(0, 2, 1)#给特征加权重.permute(0, 2, 1, 3)
            A2 = model.attention(B) # [B, n, K]
            A2 = torch.transpose(A2, 1, 2)
            A2 = F.softmax(A2, dim=-1) # [B, K, n]
            outputs = torch.bmm(A2, B) # [B, K, 512]
            #print(outputs.shape)
            #raise ValueError("已通过")
    ...
    # 提取颜色  
    label_list = [label_to_color[label] for label in flat_label_list]
    ...
    # 绘制每个点的位置
    sc = ax.scatter(tsne_results[:, 0], tsne_results[:, 1], tsne_results[:, 2], c=label_list, cmap='viridis')
    # 添加颜色条
    plt.colorbar(sc)
    # 设置坐标轴标签
    ...
    df = pd.DataFrame(tsne_results, columns=['Component 1', 'Component 2', 'Component 3'])  
    df['Label'] = label_list  
    # Save DataFrame to a CSV file  
    df.to_csv('/mnt/data3/heatmap/tsne/tsne_results.csv', index=False) 
    # 保存为PDF
    plt.savefig('/mnt/data3/heatmap/tsne/tsne_plot.pdf')